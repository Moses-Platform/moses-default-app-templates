...
// webhook handler gets a nil sink and the (empty) app routes a nil *sql.DB —
// the route tests below use a DELETE so the dispatch returns 405 (route
...
func TestRoutes_NoBasePath(t *testing.T) {
	mux := newTestMux("")
	if !routeRegistered(mux, http.MethodDelete, "/api/v1/webhooks/chat-complete") {
		t.Error("/api/v1/webhooks/chat-complete not registered when no base path is set")
	}
}
...
func TestRoutes_SubPathMount(t *testing.T) {
	...
	if !routeRegistered(mux, http.MethodDelete, "/apps/t/x/api/v1/webhooks/chat-complete") {
		t.Error("/apps/t/x/api/v1/webhooks/chat-complete not registered under the base path")
	}
