 ...
 *
 * HOW TO USE IT: move the component where it belongs (a page under
 * src/pages/, or straight into src/App.tsx), rename Thing/things to your real
 * resource, then delete this file (and src/api/example.ts).
 */
import { useQueryClient } from '@tanstack/react-query';
import { getErrorMessage } from './api/client';
import { useMosesChatComplete } from './moses/hostMessages';
import { exampleQueryKeys, useCreateThing, useThings } from './api/example';
...
export function Things() {
  ...
  // A chat_prompt completion that wrote new rows invalidates the SAME key the
  // read uses — that is why the key lives in one factory.
  useMosesChatComplete(() => {
    void qc.invalidateQueries({ queryKey: exampleQueryKeys.things });
  });
