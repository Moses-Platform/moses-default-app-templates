...
if [ -n "$MOSES_BASE_PATH_PREFIX" ]; then
  ...
  MOSES_SUBPATH_LOCATION_BLOCK="location ^~ ${MOSES_BASE_PATH_PREFIX}/api/ {
    ...
    # CHAT-pxeo.12: the X-Moses-* REQUEST headers are forwarded by nginx's
    # default proxy behaviour — no directive needed (proxy_pass_header
    # would be a no-op: it controls upstream RESPONSE headers). Backend
    # reads MOSES_TENANT_ID from env for storage keys; the forwarded
    # header only drives the backend's 403 cross-check on writes.
  }
