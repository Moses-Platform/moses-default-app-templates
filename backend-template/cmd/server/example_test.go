...
// it — never a second bare-path mount at "/api/v1/things" as well.
//
...
//
// (adding the import "github.com/moses-platform/backend-template/internal/handler"
...
// exampleLogDemoEndpoints is what logDemoEndpoints looks like once you have
// routes. Keep it in sync with the registrations above.
func exampleLogDemoEndpoints(displayURL, basePath string) {
	log.Printf("Things API: %s%s/api/v1/things", displayURL, basePath)
}
