...
// Contract-only test surface: the template ships no *application* API routes
// (/health and the OpenAPI spec are always mounted — see main.go), so these
// tests lock the Moses plumbing — health + OpenAPI dual-mount and the
// spec's canonical shape. As you add routes, add per-route tests here and
// keep TestOpenAPISpec_MuxConsistency green.
