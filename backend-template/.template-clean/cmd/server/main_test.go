...
// Contract-only test surface (post clean_out_template.sh): the demo Item
// routes are gone, so these tests lock the Moses plumbing — health +
// OpenAPI dual-mount and the spec's canonical shape. As you add routes,
// add per-route tests here (see the pristine template's TestRoutes_* for
// the pattern) and keep TestOpenAPISpec_MuxConsistency green.
...
func TestOpenAPISpec_ServersBasePathFree(t *testing.T) {
	assertOpenAPIServersBasePathFree(t, openAPISpecPath())
}
