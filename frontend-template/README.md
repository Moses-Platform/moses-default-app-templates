...
│   ├── moses-browser-logger.test.ts
│   └── vite-env.d.ts        # Vite type definitions
├── public/
│   └── favicon.svg          # Moses branding icon
├── index.html               # moses-base-path meta + no-flash theme init
...
```typescript
// src/api/client.ts — add a typed read + write
export interface Thing { id: string; name: string }
export const getThings = (signal?: AbortSignal) =>
  fetchAPI<{ things: Thing[] }>('api/v1/things', signal) // relative path, no leading '/'
...
// src/api/hooks.ts — expose them as hooks
...
export function useCreateThing() {
  ...
  return useMutation({
    ...
    onSettled: () => qc.invalidateQueries({ queryKey: queryKeys.things }), // never refetch() by hand
  })
}
```
...
2. Review the other templates in this repository for reference implementations
