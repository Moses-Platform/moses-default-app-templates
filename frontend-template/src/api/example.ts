...
// the backend handler encodes map[string]any{"things": things} and the OpenAPI
// response schema says the same. All three layers must agree.
//
// No CSRF header on the write: the template's guard is Sec-Fetch-Site-based,
// so a same-origin mutation passes with no token.
...
export function useCreateThing() {
  ...
  return useMutation({
    ...
    // Invalidate on settle so the list reconciles with server truth — never
    // call refetch() by hand.
