# Declaring Runtime Secrets — fullstack-unified

External runtime secrets that this single-binary app reads (third-party API
keys, encryption keys, JWT signing keys) are declared once in
`moses-app.config.json` and supplied per deployment by a tenant admin via the
Apps-page shield modal. The deploy gate pauses at `awaiting_secrets` until
every `required` value is present, then auto-resumes the Helm phase by
reusing the already-built image — no rebuild on supply.

The embedded static frontend does NOT read secrets directly — it calls the
binary's own API endpoints, which in turn read the env vars below.
...
1. Copy the env-var read + fail-fast snippet into `main.go` (or a helper).
