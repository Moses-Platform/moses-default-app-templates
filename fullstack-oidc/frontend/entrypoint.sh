...
if [ -n "$MOSES_BASE_PATH_PREFIX" ]; then
  ...
  MOSES_SUBPATH_LOCATION_BLOCK="location ^~ ${MOSES_BASE_PATH_PREFIX}/api/ {
    ...
    # The backend reads MOSES_TENANT_ID from env for storage keys; the
    # forwarded header drives the 403 cross-check on writes.
  ...
  location ^~ ${MOSES_BASE_PATH_PREFIX}/auth/ {
    # CHAT-t5d1u.28.8: OIDC handshake routes are served by the Go
    # backend's vendored oidcauth middleware. proxy_pass with NO URI
    # part forwards the full MOSES_BASE_PATH-prefixed path unchanged so
