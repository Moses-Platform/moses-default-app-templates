 ...
 *
 * HOW TO USE IT: move the component into your own page, register a route for
 * it in src/App.tsx, rename Thing/things to your real resource, then delete
 * this file (and src/api/example.ts).
 */
...
export function Things() {
  // Auth truth comes from useAuth(); the query is gated on the authenticated
  // phase so it never fires while anonymous.
  const { phase } = useAuth();
  // Data comes from the TanStack Query hook — never a useEffect fetch, never a
  // raw fetch() in a component.
  const { data, isPending, isError, error } = useThings(phase === 'authenticated');
