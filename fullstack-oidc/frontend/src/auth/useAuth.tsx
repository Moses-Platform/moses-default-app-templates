...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // `via` records HOW the (server-owned) identity was obtained, for the
  // demo narration; `probe` tracks the silent-SSO attempt's lifecycle so
  // the derived phase can hold `loading` until it settles.
  const [via, setVia] = useState<AuthVia>('none');
  const [probe, setProbe] = useState<'idle' | 'pending' | 'done'>('idle');
  ...
  // The anonymous phase is only surfaced once the silent probe has
  // SETTLED. Every page load whose session cookie lapsed used to flash
  // the "Sign in" state for the probe's duration — on an embedded app
  // that read as "logged out on every refresh" even though the probe
  // silently signed the user back in moments later.
