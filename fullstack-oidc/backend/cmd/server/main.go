...
//
// The built-in route surface is /api/v1/me (PROTECTED session
// introspection). Your own routes go in registerDemoRoutes
// (demo_routes.go) — see skills/usage.md for the contract. /health and
// the OpenAPI spec are always public.
package main
...
func buildMux(basePath string, oidcEnabled bool, db *sql.DB) *http.ServeMux {
	...
	// Your API routes live in demo_routes.go: one call site, so route
