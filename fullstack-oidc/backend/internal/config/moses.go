...
//
// This mirrors fullstack-showcase/internal/config — kept identical so
// the OIDC template's tenant story matches the rest of the fleet.
package config
...
// SelfTenantID returns the deploy-pinned tenant id from env.
...
// deployed pod (MOSES_DEPLOYED=1) when MOSES_TENANT_ID is unset.
func Validate() {
	resolveSelfTenant()
