...
func TestCORS_NoAllowlist_EmitsNoHeaders(t *testing.T) {
	...
	rec := corsProbe(t, http.MethodGet, "https://evil.example.com")
	...
	if rec.Code != http.StatusOK {
		t.Errorf("request must still be served same-origin, got %d", rec.Code)
	}
}
...
func TestCORS_AllowlistedOrigin_EchoedWithCredentials(t *testing.T) {
	...
	rec := corsProbe(t, http.MethodGet, "https://b.example.com")
...
func TestCORS_WildcardEntry_NeverCombinesWithCredentials(t *testing.T) {
	t.Setenv("CORS_ALLOWED_ORIGINS", "*")
	rec := corsProbe(t, http.MethodGet, "https://any.example.com")
	if got := rec.Header().Get("Access-Control-Allow-Origin"); got != "*" {
...
func TestCORS_Preflight_AllowedOrigin204(t *testing.T) {
	t.Setenv("CORS_ALLOWED_ORIGINS", "https://a.example.com")
	rec := corsProbe(t, http.MethodOptions, "https://a.example.com")
...
func TestCORS_Preflight_DeniedOriginPassesThrough(t *testing.T) {
	t.Setenv("CORS_ALLOWED_ORIGINS", "https://a.example.com")
	rec := corsProbe(t, http.MethodOptions, "https://evil.example.com")
	if got := rec.Header().Get("Access-Control-Allow-Origin"); got != "" {
