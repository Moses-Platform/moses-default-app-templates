...
// TestCookieNames_StableAcrossClientIDChurn is the regression guard for the
// app-OIDC cookie-overflow fix. The platform mints a fresh ClientID on every
// dev-track redeploy; without a stable namespace the cookie NAME churned each
// deploy, and because app cookies are Path=/ on the shared browser origin and
// never evicted, they accumulated until the Cookie header overflowed the edge
// (nginx 400). When the platform injects a stable per-(chart,track)
// CookieNamespace, two deploys with DIFFERENT ClientIDs must resolve to the
// SAME cookie name so a redeploy overwrites — not accumulates — its cookie.
func TestCookieNames_StableAcrossClientIDChurn(t *testing.T) {
	ns := "app-2837eb92-dev"
	deploy1 := Config{ClientID: "app-e60935ed-agent-599e4656-frontend", CookieNamespace: ns}
	deploy2 := Config{ClientID: "app-e60935ed-agent-0010c420-frontend", CookieNamespace: ns}

	if deploy1.sessionCookieName() != deploy2.sessionCookieName() {
		t.Fatalf("session cookie name must be STABLE across ClientID churn when CookieNamespace is set: %q != %q",
			deploy1.sessionCookieName(), deploy2.sessionCookieName())
	}
	if deploy1.stateCookieName() != deploy2.stateCookieName() {
		t.Fatalf("state cookie name must be stable across ClientID churn when CookieNamespace is set")
	}
	// The namespace must take precedence over ClientID: a namespaced config must
	// NOT collide with the legacy ClientID-derived name.
	legacy := Config{ClientID: deploy1.ClientID}
	if deploy1.sessionCookieName() == legacy.sessionCookieName() {
		t.Fatalf("namespaced cookie name must differ from the legacy sha256(ClientID) name")
	}
	// The prefix contract is preserved so the middleware still recognises it.
	if !strings.HasPrefix(deploy1.sessionCookieName(), SessionCookieName) {
		t.Fatalf("namespaced session cookie name %q must keep the %q prefix", deploy1.sessionCookieName(), SessionCookieName)
	}
}
