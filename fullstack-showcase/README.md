# Moses Fullstack Template (`fullstack-showcase`)

**Go + React + PostgreSQL scaffold with the full Moses platform integration pre-wired**

A multi-service fullstack scaffold: nginx-served React 19 frontend + Go backend + per-app Postgres, deployed as a single multi-service Helm chart. Every Moses platform contract (sub-path routing, tenant pinning, OpenAPI discovery → MCP tool generation, CSP/embedding, browser-log forwarding) is wired and tested — the application code is yours to write.

## What ships wired

### Moses platform integration
- **OpenAPI Discovery**: the backend serves `api/openapi.json`; the platform probes it after deploy and generates one MCP tool per `operationId`
- **Multi-Tenancy**: deploy-pinned tenant identity + a 403 caller cross-check on writes
- **Sub-path routing**: routes registered once under `MOSES_BASE_PATH`; nginx + Vite base + router basename all agree
- **Deployment Pipelines**: agent-execution and workspace-tool pipelines with in-cluster image builds
...
- **Routing**: react-router-dom (a placeholder home page + a 404 route — add yours under `src/pages/`)
- **Server**: nginx with runtime env var substitution
- **Styling**: Moses design tokens (4px grid, system font stack, indigo primary)

### Backend Stack
- **Language**: Go 1.26 (go.mod `1.26.4`)
...
and uses it as the authoritative storage/lookup key for every
tenant-scoped table you add. The `X-Moses-Tenant-ID` header is preserved as caller context
...
**Tenant column typing (CHAT-suvi).** Declare the `tenant_id` column on the
tables you add as `TEXT NOT NULL DEFAULT ''`, not `UUID` — the local-dev
sentinel string `local-dev` is not a UUID and a `UUID NOT NULL` column
rejects it. If you later tighten a column to `UUID NOT NULL`, any rows
written under the sentinel must first be re-tenanted (or purged) — otherwise
the type cast rejects them. The boot-time `MigrateTenant` hook in
`internal/database/migrate_demo.go`, which rewrites legacy
`'local-dev' | 'default' | ''` rows to the deploy-pinned tenant, is where
that migration goes.
...
6. **MCP Tools Generated** — one per OpenAPI `operationId`, named
   `workspace_<toolKey>_<operationId>`. `backend/api/openapi.json` ships with
   `"paths": {}`, so no tools are registered until you add paths. `/health` is
   deliberately NOT in the spec, so no phantom health tool is ever registered.
...
├── backend/
│   ├── Dockerfile                 # Multi-stage Go build (golang:1.26.4-alpine → alpine:3.21)
│   ├── go.mod                     # Go 1.26.4, pgx v5
│   ├── cmd/server/
...
│   │   │   └── migrate_demo.go    # Your schema + the tenant-rewrite hook
│   │   ├── handler/               # HTTP handlers — add yours here
│   │   │   ├── health.go          # /health (dual-mounted)
│   │   │   ├── openapi.go         # Serves embedded spec
│   │   │   └── tenant.go          # enforceTenantMatch 403 cross-check (plumbing)
│   │   └── middleware/            # Moses headers, CSRF (vendored), CORS, logging
│   └── api/openapi.json           # OpenAPI 3.0.3 spec, "paths": {} (embedded via api/api.go)
├── frontend/
...
│   │   ├── App.test.tsx           # Routing/render smoke test
│   │   ├── App.css                # Moses design tokens
│   │   ├── styles/theme.css
│   │   ├── api/                   # TanStack Query data layer
│   │   │   ├── client.ts          # Typed fetch transport (+ getErrorMessage)
│   │   │   ├── hooks.ts           # useQuery/useMutation hooks (add yours)
│   │   │   ├── queryKeys.ts       # Central query-key factory
│   │   │   ├── example.ts         # Worked `things` client+keys+hooks (type-checked, tree-shaken)
...
├── skills/
│   ├── usage.md                 # START HERE: env contract + patterns
...
1. **Create Handler**: `backend/internal/handler/things.go` — the worked
   `ThingsHandler` example ships as real, CI-compiled code in
   `backend/internal/handler/example_test.go` (a `_test.go` file so it can
   never reach the binary); move it into its own file
2. **Register Route**: `backend/cmd/server/demo_routes.go` → `registerDemoRoutes`,
   ALWAYS under the `basePath` prefix and exactly ONCE
   (`mux.HandleFunc(basePath+"/api/v1/things", h)`) — a second bare-path
   mount breaks the sub-path deploy contract and fails `cmd/server/main_test.go`
3. **Update OpenAPI**: Add to `backend/api/openapi.json` with an `operationId`,
   using a path key RELATIVE to `servers[0].url` (`/things`, never
   `/api/v1/things`)
4. **Add Data Hook**: `frontend/src/api/hooks.ts` (via `client.ts` + `queryKeys.ts`)
...
```go
// internal/middleware/moses_headers.go — SelfTenantID is env-pinned,
// CallerTenantID comes from the header (audit + 403 cross-check ONLY).
...
```go
// Always filter by the deploy-pinned self tenant — NEVER by the header value
query := `SELECT * FROM resources WHERE id = $1 AND tenant_id = $2`
db.Query(ctx, query, resourceID, mosesCtx.SelfTenantID)
```

### OpenAPI → MCP Tools

```json
// api/openapi.json — servers[0].url is "/api/v1", so path keys are
// RELATIVE to it. "/api/v1/things" here would double-prefix.
{
  "paths": {
    "/things": {
      "get": {
        "operationId": "listThings",  // ← Tool name generated
        "summary": "List things"
      }
    }
  }
...
// Moses generates: workspace_<toolKey>_listThings
```
...
# 4. Check frontend→backend communication (nginx proxies /api/* to the backend)
kubectl port-forward -n {namespace} svc/agent-deployed-app-frontend 8080:8080
curl http://localhost:8080/api/openapi.json

# 5. Verify OpenAPI discovery on the backend itself
kubectl port-forward -n {namespace} svc/agent-deployed-app-backend 8080:8080
curl http://localhost:8080/api/openapi.json
```
...
**Symptom**: `mosesCtx.CallerTenantID` / `UserID` are always empty in handler logs
...
### This Template
- **`skills/usage.md`** — START HERE: what ships, the full env-var contract, and the tenancy model
- **`skills/api-integration.md`** — Moses integration patterns (headers, base path, OpenAPI→MCP)
- **`skills/secrets-tutorial.md`** — declaring runtime secrets in `moses-app.config.json`
