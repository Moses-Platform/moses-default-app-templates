...
// Direct unit tests for enforceTenantMatch. Unlike notes_tenant_test.go
// (which routes through middleware.MosesHeaders and therefore inherits
// config.SelfTenantID's process-wide sync.Once resolution — whichever test
// resolves it first pins the value for the whole binary), these construct
// the middleware.MosesContext literal directly. No env-derived global is
// touched, so the assertions hold regardless of test ordering and they
// keep protecting the helper after the demo notes handlers are removed by
