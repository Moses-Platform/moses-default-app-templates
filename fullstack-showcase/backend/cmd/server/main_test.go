...
// key that itself starts with /api/ would double-prefix — exactly the bug
// this template shipped (servers:/api/v1 PLUS /api/v1/...-rooted paths
// registered /api/v1/api/v1/... and the proxy 404'd on every tool call).
// /health must not be listed either (phantom tool at /api/v1/health).
...
// answers 404 for a missing resource (and the notes handlers would
// dereference the nil test *sql.DB).
