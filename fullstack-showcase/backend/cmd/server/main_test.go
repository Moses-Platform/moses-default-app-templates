...
// Contract-only tests: they pin the Moses path contract (health + openapi
// dual-mount, base-path-free servers[], spec↔mux consistency) and stay
// green with ZERO *application* API routes registered (the plumbing routes —
// /health and /api/openapi.json, each at both the canonical and base-path
// mount — are always registered). When you add routes in
// demo_routes.go's successor + api/openapi.json, these tests start
// checking them automatically — do not delete this file.
...
// nil *sql.DB — the contract tests never touch a real database.
...
// key that itself starts with /api/ would double-prefix. /health must not
// be listed either (phantom tool at /api/v1/health).
...
// platform's workspace-tool proxy requests. Trivially green while paths is
// empty; it bites as soon as you document a route you forgot to register
// (or vice versa — keep both sides in sync).
